...
export default function NewsPageForm({
	...
	const form = useForm<FormValues>({
		...
		defaultValues: {
			...
			csr_desc: "",
			hero_title_en: "",
			hero_desc_en: "", // Default value baru
			news_title_en: "",
			news_desc_en: "",
			csr_title_en: "",
			csr_desc_en: "",
	...
	useEffect(() => {
		if (isEditMode && initialData) {
			...
			reset({
				// Hero Content sekarang ambil dari relasi newsPageId / newsPageEn
				hero_title: initialData.newsPageId?.hero_title || "",
				hero_desc: initialData.newsPageId?.hero_desc || "",
				news_title: initialData.newsPageId?.news_title || "",
				news_desc: initialData.newsPageId?.news_desc || "",
				csr_title: initialData.newsPageId?.csr_title || "",
				csr_desc: initialData.newsPageId?.csr_desc || "",

				// English Content
				hero_title_en: initialData.newsPageEn?.hero_title || "",
				hero_desc_en: initialData.newsPageEn?.hero_desc || "",
				news_title_en: initialData.newsPageEn?.news_title || "",
				news_desc_en: initialData.newsPageEn?.news_desc || "",
				csr_title_en: initialData.newsPageEn?.csr_title || "",
				csr_desc_en: initialData.newsPageEn?.csr_desc || "",
			});
		}
	...
	const onSubmit = async (data: FormValues) => {
		...
		const url =
			isEditMode ?
				`${process.env.NEXT_PUBLIC_API_URL}/page/news/${initialData.id}`
			:	`${process.env.NEXT_PUBLIC_API_URL}/page/news`;
			...
			<form onSubmit={form.handleSubmit(onSubmit, (errors) => {
				...
				<Tabs defaultValue="id" className="w-full">
					...
					<TabsContent value="id" className="space-y-6">
						...
						<SectionGroup title="News Section (ID)" icon={FileText}>
							<FormInput
								id="news_title"
								label="News Title"
								register={form.register}
						...
						<SectionGroup title="CSR Section (ID)" icon={Briefcase}>
							<FormInput
								id="csr_title"
								label="CSR Title"
								register={form.register}
								error={errors.csr_title}
							/>
							<FormInput
								id="csr_desc"
					...
					<TabsContent value="en" className="space-y-6">
						...
						<SectionGroup title="News Section (EN)" icon={FileText}>
							<FormInput
								id="news_title_en"
								label="News Title (EN)"
								register={form.register}
						...
						<SectionGroup title="CSR Section (EN)" icon={Briefcase}>
							<FormInput
								id="csr_title_en"
								label="CSR Title (EN)"
								register={form.register}
								error={errors.csr_title_en}
							/>
							<FormInput
								id="csr_desc_en"
								label="CSR Description (EN)"
								textarea
								register={form.register}
								error={errors.csr_desc_en}
							/>
						</SectionGroup>
					</TabsContent>
				</Tabs>
