						...
						{data.map((item) => (
							...
							>
								...
								<div className="relative aspect-video bg-slate-100 overflow-hidden border-b">
									{item.heroSlides?.[0]?.bg_image ?
										<Image
											src={item.heroSlides[0].bg_image}
											alt="Hero Preview"
											fill
									...
									<div className="space-y-3">
										<div>
											<h3 className="font-bold text-lg text-slate-900 line-clamp-1">
												{item.heroSlides?.[0]?.title_id || "Untitled Page"}
											</h3>
											<p className="text-sm text-slate-500 line-clamp-2 mt-1">
												{item.heroSlides?.[0]?.desc_id || "No description provided."}
											</p>
										</div>
