...
import { Edit, Trash2, Plus, CastIcon } from "lucide-react";
...
export default function PartnerList() {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_API_URL}/partner/${id}`,
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-6">
                <div>
                    ...
                    <p className="text-slate-500 mt-1 flex items-center gap-2">
                        Kelola daftar partner (Insurance & Funding).
                    </p>
                </div>
                <Link href="/admin/content/partner/create">
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 bg-slate-50 rounded-2xl border border-dashed border-slate-300 text-center">
                    <div className="bg-white p-4 rounded-full shadow-sm mb-4">
                        <CastIcon className="h-8 w-8 text-slate-400" />
                    </div>
                    ...
                    <Link href="/admin/content/partner/create">
                        <Button variant="outline">Tambah Partner Pertama</Button>
                    </Link>
                </div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {items.map((item) => (
                        ...
                        >
                            {/* LOGO */}
                            <div className="relative aspect-video bg-slate-50 overflow-hidden border-b flex items-center justify-center p-6">
                                {item.logo_image ? (
                                    <Image
                                        src={item.logo_image}
                                    ...
                                    <CastIcon size={32} className="text-slate-300" />
                                )}

                                {/* Category Badge */}
                                <div className="absolute top-2 right-2">
                                    <Badge
                                        variant={item.category === "INSURANCE" ? "default" : "secondary"}
                                        className="text-xs"
                                    >
                                        {item.category}
                                    </Badge>
                            ...
                            <CardFooter className="p-3 bg-slate-50/50 border-t flex gap-2">
                                <Link href={`/admin/content/partner/update/${item.id}`} className="flex-1">
                                    <Button
                                        variant="outline"
                                        size="sm"
